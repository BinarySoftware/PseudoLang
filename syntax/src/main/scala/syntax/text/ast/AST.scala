 ...
 *
 * @param tags     - If exists, holds [[AST#Tags]] to documented text
 * @param synopsis - If exists, holds [[AST#Synopsis]] of documented text
 * @param body     - If exists, holds [[AST#Body]] of documented text
 */
final case class AST(
                      tags: Option[AST.Tags],
                      synopsis: Option[AST.Synopsis],
                      body: Option[AST.Body]
                    ) extends AST.Symbol {
  val repr: Repr.Builder = R + tags + synopsis + body
  val html: AST.HTML = Seq(
    HTML.div(htmlCls())(tags.html)(synopsis.html)(body.html)
  )
}

object AST {
  def apply(): AST           = AST(None, None, None)
  def apply(tags: Tags): AST = AST(Some(tags), None, None)
  def apply(synopsis: Synopsis): AST =
    AST(None, Some(synopsis), None)
  def apply(synopsis: Synopsis, body: Body): AST =
    AST(None, Some(synopsis), Some(body))
  def apply(tags: Tags, synopsis: Synopsis): AST =
    AST(Some(tags), Some(synopsis), None)
  def apply(tags: Tags, synopsis: Synopsis, body: Body): AST =
    AST(Some(tags), Some(synopsis), Some(body))
